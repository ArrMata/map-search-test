				...
				(
					<>
						<div className="modal-box max-w-lg bg-white rounded-md p-3">
							...
							<div>
								{activeLocation.details?.description ? <p className="text-black px-1 mt-6">{activeLocation.details.description}</p> : null}
										...
										{activeLocation.images ?
											activeLocation.images.map((imageUrl) =>
												<img className="rounded-sm"
													src={imageUrl} key={imageUrl}
													onError={(e) => e.target.src = `https://placehold.co/500/7f1d1d/white/?text=Broken+Image`}
